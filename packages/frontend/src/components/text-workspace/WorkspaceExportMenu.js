...
export default function WorkspaceExportMenu({ onExport, options = defaultExportOptions }) {
  return (
    <div className="absolute right-0 z-20 mt-2 grid w-44 overflow-hidden rounded-lg border border-slate-200 bg-white p-1 shadow-[0_18px_42px_rgba(15,23,42,0.16)]">
      {options.map((option) => (
        <button
          className="flex items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-bold text-slate-700 transition hover:bg-violet-50 hover:text-violet-700 focus:outline-none focus:ring-2 focus:ring-violet-100"
          key={option.value}
