  ...
  return (
      <main className="dashboard-content">
        <header className="dashboard-topbar">
          ...
          <div className="search-box">⌕ <span>Search anything...</span><kbd>⌘ K</kbd></div>
        ...
        <div className="title-row">
          ...
          <Link className="btn btn-primary link-button" href="/editor">＋ New Project</Link>
        </div>
        ...
        <div className="project-grid">
          {projects.map(([icon, title, type, updated, tone]) => (
            <Link href={title === "Marketing Banner" ? "/collaboration" : "/editor"} key={title}>
              <ProjectCard icon={icon} title={title} type={type} updated={updated} tone={tone} clickable />
            </Link>
          ))}
        </div>
